...
function sanitize(reply) {
  ...
  if (dirty) {
    ...
    return 'Main gali-galoch nahi karti. Chalo pyaar se baat karte hain — kya poochhna hai?';
  }
...
async function getSofiyaReply(userId, chatId, message, opts = {}) {
  ...
  const marker = mood === 'sad' ? 'sad' : mood === 'romantic' || matureRequested ? 'love' : 'chatbot';
