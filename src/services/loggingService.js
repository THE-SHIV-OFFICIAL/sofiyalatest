...
function userLink(u) {
  ...
  return `<a href="tg://user?id=${u.id}">${name}</a>${username} (<code>${u.id}</code>)`;
}
...
function chatLink(c, inviteLink) {
  if (!c) return '—';
  if (c.username) return `https://t.me/${esc(c.username)}`;
  if (inviteLink) return esc(inviteLink);
...
async function logEvent(type, data = {}) {
  ...
  const header = `${ic ? `${ic} ` : ''}<b>${title}</b>${headerChat}`;
