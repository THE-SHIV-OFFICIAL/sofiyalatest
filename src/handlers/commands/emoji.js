...
const { PACK_LINKS } = require('../../utils/premiumEmoji');
...
async function emojiPacks(ctx) {
  return safeReply(ctx,
    `<b>Configured premium emoji packs</b>\n\n` +
    PACK_LINKS.map((url, index) => `${index + 1}. ${url}`).join('\n') +
    `\n\nPack URL ko message emoji ki tarah render nahi kiya ja sakta; individual custom emoji IDs required hain.`);
}
